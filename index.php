...
if(isset($_POST['login']))
  {
    ...
    $query=mysqli_query($con,"select ID from tbladmin where  UserName='$user' && Password='$password' ");
    $admincheck=mysqli_fetch_array($query);
    $query=mysqli_query($con,"select ID from tbluser where  UserName='$user' && Password='$password' ");
    $usercheck=mysqli_fetch_array($query);
    if($admincheck>0){
        $_SESSION['aid']=$admincheck['ID'];
        $_SESSION['user_type']="admin";
        header('location:dashboard.php');
    } elseif($usercheck>0){
        $_SESSION['aid']=$usercheck['ID'];
...
<head>
    ...
    <link href="dist/css/custom.css?v=<?php echo uniqid(); ?>" rel="stylesheet" type="text/css">
</head>
...
<body>
    ...
    <div class="hk-wrapper">
        ...
        <div class="hk-pg-wrapper hk-auth-wrapper">
            ...
            <a class="d-flex auth-brand align-items-center" href="javascript:void();">
                <span class="text-white font-30">Fertilizer & Seeds Shop</span>
            </a>   
            </header>
